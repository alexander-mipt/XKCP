...
void init_internals_default(internal_t *s) {
  s->LSFR_state = 0x01;
  s->LFSR = true;
  s->num_rounds = 24;
}
...
void KeccakF1600_StatePermute_Dev(void *state, const internal_t *params) {
    unsigned int round, x, y, j, t;
    uint8_t LFSRstate = params->LSFR_state;

    assert(params->num_rounds <= 24);

    for(round=0; round<params->num_rounds; round++) {
        PRINT(dprintf(STDOUT_FILENO, "round: %d/%d\n", round, params->num_rounds);)
        {   /* === θ step (see [Keccak Reference, Section 2.3.2]) === */
            tKeccakLane C[5], D;
        ...
        {   /* === ι step (see [Keccak Reference, Section 2.3.5]) === */
            if (!params->LFSR)
                continue;
            PRINT(dprintf(STDOUT_FILENO, "LSFR step\n");)
            for(j=0; j<7; j++) {
                unsigned int bitPosition = (1<<j)-1; /* 2^j-1 */
                if (LFSR86540(&LFSRstate))
                    XORLane(0, 0, (tKeccakLane)1<<bitPosition);
...
void Keccak_Dev(unsigned int rate, unsigned int capacity, const unsigned char *input, unsigned long long int inputByteLen, unsigned char delimitedSuffix, unsigned char *output, unsigned long long int outputByteLen, const internal_t* params) {
    ...
    while(inputByteLen > 0) {
        ...
        if (blockSize == rateInBytes) {
            KeccakF1600_StatePermute_Dev(state, params);
            blockSize = 0;
        }
    }
