...
void KeccakF1600_StatePermute_Dev(void *state, int rounds) {
    unsigned int round, x, y, j, t;
    uint8_t LFSRstate = 0x01;
...
void Keccak_Dev(unsigned int rate, unsigned int capacity, const unsigned char *input, unsigned long long int inputByteLen, unsigned char delimitedSuffix, unsigned char *output, unsigned long long int outputByteLen, int rounds) {
    uint8_t state[200];
    unsigned int rateInBytes = rate/8;
    unsigned int blockSize = 0;
    unsigned int i;
    ...
    while(inputByteLen > 0) {
        ...
        if (blockSize == rateInBytes) {
            KeccakF1600_StatePermute_Dev(state, rounds);
            blockSize = 0;
        }
    }
